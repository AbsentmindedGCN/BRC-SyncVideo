...
namespace SyncVideo
{
    public sealed class SyncVideoConfig
    {
        ...
        public SyncVideoConfig(ConfigFile config, ConfigFile advancedConfig, string pluginLocation)
        {
            ...
            HideNativeLobbyUi = config.Bind("Phone", "HideNativeLobbyUi", true, "Hide ACN's native lobby UI by default.");
            SuppressAFK = config.Bind("Phone", "Suppress AFK", true, "Hide AFK animations for yourself and other players while in a Sync Video lobby.");
            ...
            DefaultVolume = config.Bind("Video", "Default Volume", 90, "Starting volume level (0–100). Automatically rounds to increments of 10 in-game.");
